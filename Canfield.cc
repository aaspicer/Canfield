...
{
    cout << "[q] quit [N] new game [m] move [d] deal a card [r] redo move" 
<< endl;
...
// Performs the same move as the previous time.
void Canfield::redo()
{
    int card;  
    ...
    {
        cout << "Movement not permitted. Enter any key to continue.";
        cin.get();
        while (cin.get() != '\n') ; // Clear the line
    }
...
// Adds card onto empty tableau from the reserve
void Canfield::updateTableau()
{
    int resCard;
    
    if(!reserve.isEmpty())
    {
        resCard = reserve.top();    // Gets number on card
        reserve.pop();              // Removes card from reserve
        tableau[fromId].push(resCard); // Adds card to tableau   
    }
} // Canfield::updateTableau()

// Checks to see if the player has won
bool Canfield::checkWin()
{
    if (score == 52)
    {
        ch cont;
        cout << "You have won" << endl;
        //cout << "Would you like to cont: y/n";
        //cin << cont;
        //if (cont == 'n')
           // quit = true; 
                            //was unnecessary
